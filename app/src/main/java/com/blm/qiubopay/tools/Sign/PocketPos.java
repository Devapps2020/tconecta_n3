...
import com.blm.qiubopay.tools.StringUtils;
...
public class PocketPos {
	...
	public static final byte FRAME_TOF_EMV = 0x45;
	public static final byte FRAME_TOF_PWD = 0x4D;
	public static final byte FRAME_TOF_PROCESSDATA = 0x50;
	public static final byte FRAME_TOF_MSR = 0x48;
	public static final byte FRAME_TOF_ICC = 0x4E;
	public static final byte FRAME_TOF_DISPLAY = 0x4F;
	
	public static final byte FRAME_TOF_PICC = 0x40;

	public static final byte TAG_DISPLAY_ALIGN_CENTER = 0x01;
	public static final byte TAG_DISPLAY_ALIGN_RIGHT = 0x02;
	public static final byte TAG_DISPLAY_ALIGN_LEFT = 0x03;

	public static final byte TAG_ICC_OPEN_SLOT = 0x01;
	public static final byte TAG_ICC_SEND_COMMAND = 0x02;
	public static final byte TAG_ICC_CLOSE_SLOT = 0x03;
	
	public static final byte TAG_ICC_SLOT_ICC = 0x31;
	public static final byte TAG_ICC_SLOT_SAM = 0x32;
	...
	private static int intToAscii(long number, byte[] buf, int offset,
			int length) {
	...
	private static int asciiToInt(byte[] buf, int offset, int length) {
		if (buf == null || offset < 0 || length < 1 || length > 10
				|| buf.length < (offset + length))
			return -1;
	...
	private static byte[] concatByteArray(byte[] a, byte[] b, byte c[], byte[] d) {
		...
		Log.i("result ", StringUtils.toHexString(result));
	...
	private static String bytes2charsUTF8(byte[] buf, int offset, int length,
                                          boolean bigEndian) {
		int len = 0;
	...
	private static int calcCheckSumAndSpecialBytes(byte[] buf, int offset,
			int len, byte[] checkSum, int checkSumOffset) {
		byte even = 0x00;
		byte odd = 0x00;
		boolean isEven = true;
		int cnt = 0;
	...
	private static int convertSpecialBytes(byte[] srcBuff, int srcOffset,
			int srcLen, byte[] destBuff, int destOffset) {
		for (int j = srcOffset; j < srcLen; j++, destOffset++) {
			if (srcBuff[j] == B_SOF || srcBuff[j] == B_EOF
					|| srcBuff[j] == B_TRANSFER) {
	...
	public static byte[] FramePack(byte typeFrame, byte[] srcBuff,
			int srcOffset, int srcLen) {
		if (typeFrame == FRAME_NACK || typeFrame == FRAME_ACK
				|| typeFrame == FRAME_ENQ || typeFrame == FRAME_EOT) {
			return new byte[] { B_SOF, B_SOF, B_SOF, typeFrame, B_EOF, B_EOF,
					B_EOF };
		}
		if (typeFrame == FRAME_ETX) {
			return new byte[] { B_SOF, B_SOF, B_SOF, typeFrame, getDataID(),
					B_EOF, B_EOF, B_EOF };
		}
		if (typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP
				|| typeFrame == FRAME_TOF_PRINT || typeFrame == FRAME_TOF_PWD
				|| typeFrame == FRAME_TOF_PROCESSDATA
				|| typeFrame == FRAME_TOF_EMV
				|| typeFrame == FRAME_TOF_ICC
				|| typeFrame == FRAME_TOF_MSR || typeFrame == FRAME_TOF_DISPLAY
				|| typeFrame == FRAME_TOF_PICC) {
			if (srcOffset < 0 || srcLen < 1
					|| (srcOffset + srcLen) > srcBuff.length)
				return null;
			byte[] checkSum = new byte[2];
			int countSpecialBytes = calcCheckSumAndSpecialBytes(srcBuff,
					srcOffset, srcLen, checkSum, 0);
			// byte[] buffer = new byte[(1+1+1) + 1 + 1 + BYTES_OF_DATA_LENGTH +
			// countSpecialBytes + (1+1+1)];
			byte[] buffer = new byte[(1) + 1 + 1 + BYTES_OF_DATA_LENGTH
					+ countSpecialBytes + (1)]; // For P25
			...
			offset = convertSpecialBytes(srcBuff, srcOffset, srcLen, buffer,
					offset);

			offset = convertSpecialBytes(checkSum, 0, checkSum.length, buffer,
					offset);
	...
	public static byte[] FrameUnpack(byte[] srcBuff, int srcOffset, int srcLen) {
		...
		//
		if(typeFrame == (0x90 - 256)){
			return new byte[]{typeFrame};
		}
		//printing response
		if( typeFrame == FRAME_EOT && srcBuff[offset + 2] == B_EOF){
			return new byte[]{typeFrame};
		}
		if ((typeFrame == FRAME_NACK || typeFrame == FRAME_ACK
				|| typeFrame == FRAME_ENQ )
		...
		if ((typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP
				|| typeFrame == FRAME_TOF_MSR || typeFrame == FRAME_TOF_ICC
				|| typeFrame == FRAME_TOF_PWD || typeFrame == FRAME_TOF_PROCESSDATA|| typeFrame == FRAME_TOF_EMV
				|| typeFrame == FRAME_TOF_PICC)
...
//						return null;
					offset++;
					System.out.println("offset---->"+offset+" length------>"+length+" srcLength----->"+srcLen);
	...
	public static byte[] buildMSRData(byte[] track1, byte[] track2,
			byte[] track3) {
		int length = 0;
	...
	// public static byte[] toUnicodeHex(String str) {
	// String ret = "";
	// int length = str.length();
	// for (int i = 0; i < length; i++) {
	// Log.e("UTF", Integer.toHexString(str.charAt(i)));
	// char ch = str.charAt(i);
	// if (ch < 256 && ch > 32) {
	// ret += "00";
	// }
	// ret += Integer.toHexString(ch);
	// }
	// int retLength = ret.length();
	// byte[] temp = new byte[retLength / 2 + 1];
	// int i = 2;
	// while (i <= retLength) {
	//
	// Log.e("UTF", Integer.parseInt(ret.substring(i - 2, i), 16) + "");
	// temp[(i - 2) / 2] = (byte) Integer.parseInt(
	// ret.substring(i - 2, i), 16);
	// i = i + 2;
	// }
	// Log.e("UTF", ret);
	// return temp;
	...
	public static byte[] convertPrintData(String str, int offset, int length,
		...
		Log.i("formate value = ", StringUtils.toHexString(formate));
	...
	public static int TLVAppend(short tag, byte[] src, int src_offset,
			byte[] dst, int dst_offset, int length) {
		int i = dst_offset;
	...
	public static byte[] convertDisplayData(String content, int timeout) {

		byte[] buffer = null;
		char type = 3;
		String contentStr = type + String.valueOf(timeout);
		String[] contents = content.split("\n");
		int lines = contents.length;
//		byte[] contentByte = new byte[0];
		for(int i = 0; i < lines; i ++) {

//			Log.i("contents[i]", contents[i]);
			char alignFlag = contents[i].charAt(0);
//			Log.i("contents[i].charAt(0)", "TEST"+alignFlag+"TEST1");
			String lineStr = contents[i].substring(1);
			int lineLen = lineStr.length();
			char lineLenByte = (char) lineLen;
			String line = (i+1) + "" + alignFlag + lineLenByte + lineStr;
			contentStr += line;
			byte[] lineByte = line.getBytes();
//			Log.i("convertDisplayData line",
//					Tracer.dump(lineByte, 0, lineByte.length, true));
//			int contentByteSize = contentByte.length + lineByte.length;
//			contentByte = new byte[contentByteSize];
		}
		buffer = contentStr.getBytes();
//		Log.i("convertDisplayData buffer",
//				Tracer.dump(buffer, 0, buffer.length, true));
		return buffer;
	}
