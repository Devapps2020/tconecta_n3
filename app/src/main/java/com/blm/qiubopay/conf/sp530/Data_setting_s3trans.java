package com.blm.qiubopay.conf.sp530;

import android.content.Context;
import android.content.SharedPreferences;

import com.blm.qiubopay.conf.AppSectionsConstant;
...
public class Data_setting_s3trans {
    ...
    public static final int TIME_WAIT_PRESENTCARD_INMS          = (30*1000);

    public static final int DEFAULT_IDXTRANSRESULT              = 0;
    public static final int DEFAULT_MAX_BATCH                   = 10;
    public static final int DEFAULT_MINTIMEWAIT_BATCH           = 0;    // in minute unit
    public static final int DEFAULT_WAITTIMENORMALINS           = 25;
    public static final String DEFAULT_VAL_TRANSACTIONEXTRATLV  = ExtraTlvConstant.TLVTAG_SALE;
    ...
    public void refresh() {
        ...
        m_idxTransResult                = DEFAULT_IDXTRANSRESULT;//preference.getInt(KEY_IDXTRANSRESULT, DEFAULT_IDXTRANSRESULT);
        m_maxBatch                      = DEFAULT_MAX_BATCH;//preference.getInt(KEY_MAXBATCH, DEFAULT_MAX_BATCH);
        m_mintimewaitBatch              = DEFAULT_MINTIMEWAIT_BATCH;//preference.getInt(KEY_MINTIMEWAITBATCH, DEFAULT_MINTIMEWAIT_BATCH);
        m_waitTimeNormalInS             = DEFAULT_WAITTIMENORMALINS;//preference.getInt(KEY_WAITTIMENORMALINS, DEFAULT_WAITTIMENORMALINS);
        m_strTransactionExtraTlv        = DEFAULT_VAL_TRANSACTIONEXTRATLV;//preference.getString(KEY_TRANSACTIONEXTRATLV, DEFAULT_VAL_TRANSACTIONEXTRATLV);
        m_bTransactionSimulateTCPHost   = DEFAULT_VAL_TRANSACTIONSIMULATETCPHOST;//preference.getBoolean(KEY_TRANSACTIONSIMULATETCPHOST, DEFAULT_VAL_TRANSACTIONSIMULATETCPHOST);

        // Note:
        //  need to add sp530 present card wait time
        m_waitReponseTimeInMs= TIME_WAIT_PRESENTCARD_INMS+(m_waitTimeNormalInS*1000);
    }
    ...
    public void setMinTimewaitBatch(int val) {
        SharedPreferences preference=m_context.getSharedPreferences(AppSectionsConstant.Storage.PREFERENCE_CONF_SETTING, Context.MODE_PRIVATE);
        preference.edit().putInt(KEY_MINTIMEWAITBATCH, val).commit();
        refresh();
    }
