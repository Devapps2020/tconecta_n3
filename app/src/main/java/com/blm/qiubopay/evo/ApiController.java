...
public class ApiController {
    ...
    String executeCompleteSession(String sessionId, String orderId, String transactionId, String amount, String currency, String threeDSecureId, Boolean isGooglePay) throws Exception {
        GatewayMap request = new GatewayMap()
                ...
                //.set("sourceOfFunds.token", "123456789010")
        ...
        if (!response.containsKey("gatewayResponse.result") || !"SUCCESS".equalsIgnoreCase((String) response.get("gatewayResponse.result"))) {
            ...
            String code = (String) response.get("gatewayResponse.response.gatewayCode");
            String message = "ERROR AL PROCESAR LA OPERACIÓN";

            if(code != null) {
                switch (code) {
                    case "APPROVED":
                        message = "OPERACIÓN APROVADA";
                        break;
                    case "DECLINED":
                        message = "OPERACIÓN DECLINADA";
                        break;
                    case "EXPIRED_CARD":
                        message = "TARJETA EXPIRADA";
                        break;
                    case "ACQUIRER_SYSTEM_ERROR":
                        message = "ERROR DEL SISTEMA DE ADQUISICIÓN";
                        break;
    ...
    public GatewayMap createToken(String sessionId) throws Exception {

        String user = "TESTBLUELABELAPP";
        String pass = "9f30d30b7f128b266f7bcab37beae08a";

        GatewayMap request = new GatewayMap().set("sourceOfFunds.token", "123456789010");

        String jsonRequest = GSON.toJson(request);

        String jsonResponse = doJsonRequest(new URL("https://evopaymentsmexico.gateway.mastercard.com/api/rest/version/51/merchant/TESTBLUELABELAPP/session/" + sessionId), jsonRequest, "PUT", user, pass, HttpsURLConnection.HTTP_OK);
