...
class BTEventsForActivity {
    ...
    private String getBTAddressFromSharedPreferences() {
        ...
        if (m_context == null) {
            Logger.w(m_className, "getBTAddressFromSharedPreferences, m_context is NULL");
            return btAddress;
        }
        SharedPreferences preference=m_context.getSharedPreferences(PREFERENCE_ACTIVEBLUETOOTHDEVICE_ADDRESS, Context.MODE_PRIVATE);
    ...
    public void setBTAddress2SharedPreferences(String btAddress) {
        ...
        if (m_context == null) {
            Logger.w(m_className, "setBTAddress2SharedPreferences, m_context is NULL");
            return;
        }
        SharedPreferences preference=m_context.getSharedPreferences(PREFERENCE_ACTIVEBLUETOOTHDEVICE_ADDRESS, Context.MODE_PRIVATE);
    ...
    // 30/08/17 PC1708 add for set retry until connected
    public boolean setRetryTillConnect(boolean aTf) {
        if (m_reconnectThreadInst == null) {
            return false;
        }
        return m_reconnectThreadInst.setRetryTillConnect(aTf);
    }
    //==>End PC1708
